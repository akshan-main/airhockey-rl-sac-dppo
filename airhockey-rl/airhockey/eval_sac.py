...
def scripted_attacker(obs: np.ndarray) -> np.ndarray:
    """Active opponent that can serve, attack, and defend.
    ...
    opponent's. Layout matches physics.get_obs:
      [0] puck_x  [1] puck_y  [2] puck_vx  [3] puck_vy
      [4] own_x   [5] own_y   [6] own_vx   [7] own_vy
      [8] other_x [9] other_y
    ...
    Three modes:
      SERVE — puck is nearly stationary (any half). Go to the puck and
      hit it toward the opponent goal. This is the critical behavior
      that was missing: without it, 50% of episodes (where the puck
      starts on the opponent's half) are dead air.

      ATTACK — puck is moving and on our half (y > 0.5). Chase the
      puck with an approach angle that aims the contact at the
      opponent goal.

      DEFEND — puck is moving and on opponent's half (y < 0.5).
      Retreat to home y, track the puck on x.
    """
    puck_x, puck_y = float(obs[0]), float(obs[1])
    puck_vx, puck_vy = float(obs[2]), float(obs[3])
    pad_x, pad_y = float(obs[4]), float(obs[5])
    puck_speed = (puck_vx ** 2 + puck_vy ** 2) ** 0.5

    # Serve: puck is nearly still. Go to it and whack it toward the
    # opponent goal regardless of which half it's on.
    if puck_speed < 0.05:
        goal_x, goal_y = 0.5, 0.0
        dx_to_goal = goal_x - puck_x
        dy_to_goal = goal_y - puck_y
        norm = (dx_to_goal ** 2 + dy_to_goal ** 2) ** 0.5
        if norm < 1e-6:
            approach_x, approach_y = puck_x, puck_y + 0.06
        else:
            offset = 0.06
        ...
        # Attack: puck is on our half and moving. Charge it with an
        # approach angle that aims the contact at the opponent goal.
        goal_x, goal_y = 0.5, 0.0
        dx_to_goal = goal_x - puck_x
        dy_to_goal = goal_y - puck_y
        norm = (dx_to_goal ** 2 + dy_to_goal ** 2) ** 0.5
        ...
        # Defend: puck is moving toward the opponent goal. Retreat to
        # home y, track the puck on x.
        target_x = puck_x
        target_y = 0.85
