...
class DPPOArgs:
    ...
    gae_lam: float = 0.95
    clip_eps: float = 0.1
    clip_eps_base: float = 0.1
    clip_eps_rate: float = 3.0
    gamma_denoising: float = 0.99
    min_logprob_std: float = 0.1
    min_sampling_std: float = 0.1
    target_kl: float = 100.0  # effectively disabled — let PPO update freely
    seed: int = 0


def _make_league_opponent(opponent_ckpt: str, seed: int):
    """Opponent league: weaker mix so DPPO can generate positive advantages.
    30% tracker, 30% attacker, 20% SAC expert, 20% stationary."""
...
def main(args: DPPOArgs):
    ...
    pending_chain_stds = [None] * args.n_envs
    pending_chain_ab_nexts = [None] * args.n_envs
    ...
    buf_obs, buf_cc, buf_cn, buf_ts, buf_cs, buf_abn, buf_olp = [], [], [], [], [], [], []
    ...
    while n_total_steps < args.total_steps:
        for _ in range(args.rollout_steps):
            ...
            if need_sample.any():
                ...
                chain_stds = mx.stack(res.stds, axis=1)
                chain_ab_nexts = mx.stack(res.ab_nexts, axis=1)
                ...
                for k in range(K):
                    lp_k = per_step_logprob(
                        ...
                        ab_next=chain_ab_nexts[:, k],
                        std=chain_stds[:, k],
                        min_logprob_std=args.min_logprob_std,
                    )
            ...
            for env_i in range(args.n_envs):
                ...
                if advanced_ptr == 0 or bool(dones[env_i]):
                    ...
                    buf_cs.append(pending_chain_stds[env_i])
                    buf_abn.append(pending_chain_ab_nexts[env_i])
        ...
        metrics = dppo_update(
            ...
            timesteps=ts_batch, chain_stds=cs_batch, chain_ab_nexts=abn_batch,
            advantages=adv_t, returns=ret_t,
            old_logprobs=olp_batch,
            clip_eps=args.clip_eps,
            clip_eps_base=args.clip_eps_base,
            clip_eps_rate=args.clip_eps_rate,
        ...
        buf_ts.clear(); buf_cs.clear(); buf_abn.clear(); buf_olp.clear()
        buf_rewards.clear()
